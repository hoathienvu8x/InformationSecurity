...
int des_encrypt(uint8_t * input, int len, uint8_t * output, uint8_t key[8]) {
    int remain = 8 - len % 8;
    int origin_len = len;
    int after_encrypt_byte_num = (len / 8 + 1) * 8;
    uint8_t * str = malloc(after_encrypt_byte_num + 1);
...
void correct_key(uint8_t key[8]) {
    uint64_t r = char_str_to_uint64(key);
    for (int i = 0; i < 8; ++i) {
        int ones_count = 0;
        for (int j = 0; j < 7; ++j) {
            if (get_bit(r, 8 * i + j)) {
